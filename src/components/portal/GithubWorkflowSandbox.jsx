...
function ReplyBox({ repo, issue, comments, onReplied }) {
  ...
  const isEmptyDiscuss = action === "!discuss" && !info.trim();

  const handleSubmit = async (e) => {
    e.preventDefault();
    const message = info.trim();
    if (!message) {
      setError("Please enter a discussion message.");
      return;
    }
    setSubmitting(true);
    setError(null);
...
function PRRow({ pr, owner, repo, user }) {
  ...
  const alreadyPinged = pings !== null && pings.length > 0;
